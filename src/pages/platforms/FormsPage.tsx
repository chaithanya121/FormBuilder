...
import { fetchForms, createForm, deleteFormAction } from '@/store/slices/formsSlice';
  ...
  return (
    <div className={`min-h-screen ${theme === 'light' 
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className={`absolute top-20 right-20 w-72 h-72 ${theme === 'light' ? 'bg-blue-200/20' : 'bg-blue-500/10'} rounded-full blur-3xl animate-pulse`}></div>
        <div className={`absolute bottom-20 left-20 w-96 h-96 ${theme === 'light' ? 'bg-purple-200/20' : 'bg-purple-500/10'} rounded-full blur-3xl animate-pulse`} style={{ animationDelay: '2s' }}></div>
      </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className={`text-xl ${theme === 'light' ? 'text-gray-900' : 'text-white'} flex items-center gap-2`}>
                    ...
                    Your Forms ({filteredForms.length})
                  </CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <div className="text-center py-12">
                    ...
                    <p className={theme === 'light' ? 'text-gray-600' : 'text-gray-400'}>Loading forms...</p>
                  </div>
                ) : filteredForms.length === 0 ? (
                  <div className="text-center py-12">
                    <FileText className={`h-12 w-12 mx-auto mb-4 ${theme === 'light' ? 'text-gray-400' : 'text-gray-600'}`} />
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex-1">
                            <h3 className={`font-semibold text-lg mb-2 ${theme === 'light' ? 'text-gray-900' : 'text-white'} group-hover:text-blue-600 transition-colors`}>
                              {form.name}
                            </h3>
