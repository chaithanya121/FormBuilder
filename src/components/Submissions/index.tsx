...
import { formsApi, FormSubmission as ApiFormSubmission, FormData } from "@/services/api/forms";
...
const Submissions = () => {
  const [forms, setForms] = useState<LocalFormData[]>([]);
    ...
    const loadFormsData = async () => {
      try {
        ...
        if (Array.isArray(storedForms) && storedForms.length > 0) {
          const convertedForms: LocalFormData[] = storedForms.map((form: any) => ({
