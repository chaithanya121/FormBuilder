...
const Submissions = () => {
  ...
  const loadForms = async (id) => {
    try {
      ...
      await formsApi.getFormSubmissions(id).then((data) => {
  ...
  useEffect(() => {
    ...
    const storedFormsJson = formsApi.getFormNameID().then((item) => {
      ...
      if (item) {
        try {
          ...
          if (storedForms.length > 0 && !selectedFormId) {
            console.log("store", item[0]["primary_id"]);
            setSelectedFormId(item[0]["primary_id"]);
            console.log("selected_id", selectedFormId);
            loadForms(item[0]["primary_id"]);
          }
        } catch (error) {
  ...
  const handleDeleteSubmission = async (submissionId: string) => {
    ...
    try {
      let delete_val = await formsApi.deleteSubmittionData(submissionId);
      ...
      toast({
        title: "Error",
        description: "Failed to deleted Data",
        variant: "destructive",
      });
    }
  };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div className="flex items-center gap-3 w-full md:w-auto">
            <Select
              ...
              onValueChange={(value) => {
                setSelectedFormId(value), loadForms(value);
              }}
            >
